...
Custom integration for Home Assistant that calculates portfolio metrics for user-defined holdings.
...
- user-defined holdings only, with no hardcoded assets
...
2. Replace placeholder URLs in `custom_components/bagwatch/manifest.json`.
3. In Home Assistant, open HACS.
4. Open the three-dot menu and choose `Custom repositories`.
5. Paste the GitHub repository URL.
6. Select repository type `Integration`.
7. Install `Bagwatch` from HACS.
...
[
  ...
  },
  {
    "symbol": "MSCI ACWI",
    "name": "MSCI ACWI ETF",
    "provider_symbol": "ACWI",
    "asset_type": "etf",
    "quantity": 3,
    "average_buy_price": 112,
    "buy_currency": "USD"
  },
  {
    "symbol": "BTC",
...
## Supported holding fields

- `symbol`: symbol shown to the user in Home Assistant
...
- `cost_basis_base`: optional exact total cost already expressed in base currency
